...
The code below will help you obtain and manage LANDFIRE data from the [LANDFIRE Product Service](https://lfps.usgs.gov/) REST API. To do so, we use the [rlandfire package](https://github.com/bcknr/rlandfire) developed by Mark Buckner. Mark's site includes a useful tutorial and more information if you want to learn more about this package and what it does.
...
This code will: 
  1.  Load the stacked raster using terra::rast().
  2.  Split the raster into individual layers using assign().
...
::: callout-note
You’ll need the BpS and EVT attribute tables as .csv files. If you didn’t download the setup zip file, you can get them from the [LANDFIRE Program Website](https://landfire.gov) by navigating to the individualproduct page for each layer, then scrolling down to the Attribute Tables(CSV, ADD) section and selecting the CSV file. We recommend storing that file in the inputs folder within your working directory to make the code work as expected.
:::
